  ...
  for (j in (r+1):(r+J/2-1))
  {
    ...
    while(sum(abs(add))>max.tol)
    {
      ...
      if (NonUniform==T){
        ...
        FI[3,3]=FI[3,2]=FI[2,3]=-sum(ng2*X[,1]*X[,1]*apply(PQdif2^2/P2,1,sum)) #gamma
        FI[4,4]=FI[4,2]=FI[2,4]=-sum(ng3*X[,1]*X[,1]*apply(PQdif3^2/P3,1,sum)) #gamma
      ...
      bet0=bet+add[5:6]
      for (mm in 5:6){
        add[mm]=soft(bet0[mm-4],-eta/FI[mm,mm])-bet[mm-4]
      }
      for (mm in 5:6){
        bet[mm-4]=soft(bet0[mm-4],-eta/FI[mm,mm])
      }
    }
    #end of M step loop
