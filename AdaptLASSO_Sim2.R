...
ipest1 <- function(resp,m,r,eta,lam,eps =1e-3,max.tol=1e-7,NonUniform=F,gra00=gra00,grd00=grd00,grbeta00=grbeta00,mu100=mu100,mu200=mu200,mu300=mu300,Sig100=Sig100,Sig200=Sig200,Sig300=Sig300)
{
  ...
  while(max(df.a)>eps | max(df.d)>eps | max(df.beta)>eps) # max(df.Mu)>eps | max(df.Sig)>eps |
  {
    ...
    for (j in (r+1):(r+J/2-1))
    {
      ...
      while(sum(abs(add))>max.tol)
      {
        ...
        for (mm in (m+r-1):(m+r)){
          add[mm]=soft(bet0[mm-m],-(eta/(abs(betmle[mm-m])^(lam)))/FI[mm,mm])-bet[mm-m]
        }
        for (mm in (m+r-1):(m+r)){
          bet[mm-m]=soft(bet0[mm-m],-(eta/(abs(betmle[mm-m])^(lam)))/FI[mm,mm])
        }
      }
      #end of M step loop
...
eta.12=numeric(reps)
#Gammas.1=array(double(2*J*m*50),dim = c(2,2,J,50))
Betas.12=array(double(J*2*reps),dim = c(J,2,reps))
