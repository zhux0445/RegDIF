# high DIF proportion in diffenrent structure ("simple" structure)
#  beta1  beta2 
#    0      0
#    0      0
#    0      0
...
grbeta0[,1]=matrix(c(0,0,rep(1.4,18)),J,1)
...
for (aa in 1:J){
  for (rr in 1:2){
    for (cc in 1:reps)
      sparsity.t[aa,rr,cc]=ifelse(Betas.2[aa,rr,cc]==0,0,1)
  }
}

power = c(sum(sparsity.t[c(4:9,12:17),1,])/(12*reps),sum(sparsity.t[c(4:9,12:17),2,])/(12*reps))
