...
while(max(df.a)>eps & max(df.d)>eps&  max(df.beta)>eps & max(df.Sig)>eps)
{
  aold <- gra
  dold <- grd
  #gammaold=grgamma
...
ipest1 <- function(resp,m,r,eta,eps =1e-3,max.tol=1e-7,NonUniform=F,gra00=gra00,grd00=grd00,grbeta00=grbeta00,Sig00=Sig00)
{
  ...
  return(list(est=cbind(gra,grd),Beta=grbeta,Corr=Sig,iter=iter,bic=BIC,bias=Bias,RMSE=RMSE))
}
#end of function
...
###################          hessian for multidimensional case       #######################
FI <- matrix(0,m+(m-1)*2,m+(m-1)*2)
if (m==2){
  for (mm in 1:(m-1)){
    FI[mm,mm] <- -sum(ng1*(Pstar1[,mm]*Qstar1[,mm])^2*(1/P1[,mm]+1/P1[,mm+1])+ng2*(Pstar2[,mm]*Qstar2[,mm])^2*(1/P2[,mm]+1/P2[,mm+1])+ng3*(Pstar3[,mm]*Qstar3[,mm])^2*(1/P3[,mm]+1/P3[,mm+1]))
  ...
  for (mm in (m+r):(m+r-1+(m-1)*2)){
    FI[mm,mm] <- c(-sum(ng2*(Pstar2[,1]*Qstar2[,1])^2*(1/P2[,1]+1/P2[,2])),-sum(ng3*(Pstar3[,1]*Qstar3[,1])^2*(1/P3[,1]+1/P3[,2])))[mm-(m+r-1)]
    for (nn in m:(m+r-1)){
      FI[nn,mm] <- FI[mm,nn] <- c(sum(ng2*X[,nn-m+1]*Pstar2[,1]*Qstar2[,1]*(PQdif2[,1]/P2[,1]-PQdif2[,2]/P2[,2])),sum(ng3*X[,nn-m+1]*Pstar3[,1]*Qstar3[,1]*(PQdif3[,1]/P3[,1]-PQdif3[,2]/P3[,2])))[mm-(m+r-1)]
    }
    FI[mm,m-1] <- FI[m-1,mm] <- c(-sum(ng2*(Pstar2[,1]*Qstar2[,1])^2*(1/P2[,1]+1/P2[,2])),-sum(ng3*(Pstar3[,1]*Qstar3[,1])^2*(1/P3[,1]+1/P3[,2])))[mm-(m+r-1)]
  }
} else {
  for (mm in 1:(m-1)){
    FI[mm,mm] <- -sum(ng*(Pstar[,mm]*Qstar[,mm])^2*(1/P[,mm]+1/P[,mm+1]))
