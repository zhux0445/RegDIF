...
\begin{equation}
\begin{aligned}
\frac{\partial \log M}{\partial a_{jr}} =  \sum_{g=1}^{G} \sum_{y}^3 \sum_{k=1}^p (\frac{r_{jgky} q_{gr}}{P_{jky\mid q_{g}}}(\omega_{j,(k-1),y}-\omega_{jky}))
\end{aligned}
\end{equation}
...
Then, to minimize our objective function with respect to $\boldsymbol \beta$, we calculate a second-order Tylor approximation of the marginal log-likelihood, and our lasso estimator in (13) can be updated by 
...
\BlankLine
\While{$||\hat{\boldsymbol A}-\boldsymbol A_0||+||\hat{\boldsymbol D}-\boldsymbol D_0||+||\hat{\boldsymbol \beta}-\boldsymbol \beta_0||>\varepsilon_1$}{
    Calculate $n_{gy}$ and $r_{jgky}$\;
    Update $\hat{\boldsymbol \mu}$ and $\hat{\boldsymbol \Sigma}$\;
    \For {j=1,...m}{
    \While{$||\hat{\boldsymbol A_j}-\boldsymbol A_{j0}||+||\hat{\boldsymbol D_j}-\boldsymbol D_{j0}||+||\hat{\boldsymbol \beta_j}-\boldsymbol \beta_{j0}||>\varepsilon_2$}{
        Calculate $P_{jky\mid q_{g}}^{*},Q_{jky\mid q_{g}}^{*}$\;
...
\Output{$\hat{\boldsymbol A}$,$\hat{\boldsymbol D}$,$\hat{\boldsymbol \beta}$,$\hat{\boldsymbol \mu}$,$\hat{\boldsymbol \Sigma}$}
\BlankLine
\While{$||\hat{\boldsymbol A}-\boldsymbol A_0||+||\hat{\boldsymbol D}-\boldsymbol D_0||+||\hat{\boldsymbol \Gamma}-\boldsymbol \Gamma_0||>\varepsilon_1$}{
    Calculate $n_{gy}$ and $r_{jgky}$\;
    Update $\hat{\boldsymbol \mu}$ and $\hat{\boldsymbol \Sigma}$\;
    \For {j=1,...m}{
    \While{$||\hat{\boldsymbol A_j}-\boldsymbol A_{j0}||+||\hat{\boldsymbol D_j}-\boldsymbol D_{j0}||+||\hat{\boldsymbol \Gamma_j}-\boldsymbol \Gamma_{j0}||>\varepsilon_2$}{
        Calculate $P_{jky\mid q_{g}}^{*},Q_{jky\mid q_{g}}^{*}$\;
        ${a}_{jr}^{(t)}=  a_{jr}^{(t-1)}-\frac{\partial_{ a_{jr}}\log M}{\partial^2_{ a_{jr}}\log M}$\;
        ${d}_{jk}^{(t)}=  d_{jk}^{(t-1)}-\frac{\partial_{ d_{jk}}\log M}{\partial^2_{ d_{jk}}\log M}$\;
        $\hat{\boldsymbol \Gamma}_{jry}=-\frac{\mathrm{soft}(\partial_{\boldsymbol \Gamma_{jky}}\log M-\boldsymbol \Gamma_{jky}^{(t-1)}*\partial^2_{\boldsymbol \Gamma_{jky}}\log M,\eta)}{\partial^2_{\boldsymbol \Gamma_{jky}}\log M}$\;
    }
    ${a}_{jr}^{(t)*}=  {a}_{jr}^{(t)}*\sqrt{\mathrm{diag}(\hat{\boldsymbol\Sigma}_{1r})}$\;
    }
}
...
$\emph Table \ 9. \  Item \ parameter \ estimates \ by \ regularization$
...
$\emph Table \ 10. \ Item \ parameter \ estimates \ by \ mirt \ LRT$

Item Parameters |$\boldsymbol a_1$| $\boldsymbol a_2$| $\boldsymbol d$| 
------------------ | -----|------|------|------|------|------|
Bias|0.0234| 0.0060|-0.0081|
RMSE|0.1689|0.1469|0.1001|

Our regularization method has slightly better non-DIF item parameter estimates.

$\emph Table \ 11. \ Absolute \ bias \ for \ DIF \ magnitude \ recoveries \ that \ were \ true \ DIF$

Group | Omnibus DIF| Group with DIF=0.5| Group with DIF=1| 
------|------|------|------|
Regularization (exclude false negative)| 0.327 | 0.175 | 0.263 |
Regularization (include false negative)| 0.343 | 0.396 | 0.290 |
mirt LRT (include false negative)| 0.200 | 0.195 | 0.204 |
...
We run a block co-ordinate gradient descent algorithm for each group (item) with all other groups fixed. For item *j*,

denote $u$ to be the subgradient of $||\boldsymbol\tau_j^{(t-1)}+\boldsymbol \epsilon_j||_2$. We have
...
\SetAlgoLined
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{$\boldsymbol A_0$,$\boldsymbol D_0$,$\boldsymbol \beta_0$,$\boldsymbol \Gamma_0$,$\boldsymbol \mu_0$,$\boldsymbol \Sigma_0$,U,$\eta$,$\varepsilon_1$,$\varepsilon_2$}
\Output{$\hat{\boldsymbol A}$,$\hat{\boldsymbol D}$,$\hat{\boldsymbol \beta}$,$\boldsymbol \Gamma$,$\hat{\boldsymbol \mu}$,$\hat{\boldsymbol \Sigma}$}
\BlankLine
\While{$||\hat{\boldsymbol A}-\boldsymbol A_0||+||\hat{\boldsymbol D}-\boldsymbol D_0||+||\hat{\boldsymbol \Gamma}-\boldsymbol \Gamma_0||+||\hat{\boldsymbol \beta_j}-\boldsymbol \beta_{j0}||>\varepsilon_1$}{
    Calculate $n_{gy}$ and $r_{jgky}$\;
    Update $\hat{\boldsymbol \mu}$ and $\hat{\boldsymbol \Sigma}$\;
    \For {j=1,...m}{
    \While{$||\hat{\boldsymbol A_j}-\boldsymbol A_{j0}||+||\hat{\boldsymbol D_j}-\boldsymbol D_{j0}||+||\hat{\boldsymbol \Gamma_j}-\boldsymbol \Gamma_{j0}||+|\hat{\boldsymbol \beta_j}-\boldsymbol \beta_{j0}||>\varepsilon_2$}{
        Update score functions and Hessian matrix by plug $a_{j}^{(t-1)}$,$d_{j}^{(t-1)}$,$\gamma_{j}^{(t-1)}$ and $\beta_{j}^{(t-1)}$ into $P_{jky\mid q_{g}}^{*},Q_{jky\mid q_{g}}^{*}$\;
        ${a}_{jr}^{(t)}=  a_{jr}^{(t-1)}+\frac{\partial_{ a_{jr}}\log M}{\partial^2_{ a_{jr}}\log M}$\;
        ${d}_{jk}^{(t)}=  d_{jk}^{(t-1)}-\frac{\partial_{ d_{jk}}\log M}{\partial^2_{ d_{jk}}\log M}$\;
        $\hat{\boldsymbol\tau}^{(t)}_j=\hat{\boldsymbol\tau}^{(t-1)}_j+\alpha^{(t-1)}\boldsymbol \epsilon^{(t-1)}_j$\;
    }
    ${a}_{jr}^{(t)*}=  {a}_{jr}^{(t)}*\sqrt{\mathrm{diag}(\hat{\boldsymbol\Sigma}_{1r})}$\;
    ${\gamma}_{jr}^{(t)*}=  {\gamma}_{jr}^{(t)}*\sqrt{\mathrm{diag}(\hat{\boldsymbol\Sigma}_{1r})}$\;
    }
