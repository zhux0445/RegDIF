...
// [[Rcpp::export]]
Rcpp::List M_step_Adapt(int j, arma::rowvec ng, arma::mat rgk, arma::mat X, int y, int G, arma::mat yallgroup, double maxtol, arma::mat gra, arma::mat grd, arma::mat grbeta,arma::mat grbeta00, arma::cube grgamma,arma::cube grgamma00,int r, int J, int m, int eta,int lam)
{
  arma::rowvec d=grd.row(j-1);
  arma::rowvec a=gra.row(j-1);
  arma::mat gam=grgamma.slice(j-1);
  arma::mat gammle=grgamma.slice(j-1);
  arma::rowvec bet=grbeta.row(j-1);
  arma::rowvec betmle=grbeta.row(j-1);
  arma::cube Pstar=zeros<cube>(G,(m-1),y);
  arma::cube Qstar=zeros<cube>(G,(m-1),y);
  arma::cube P=zeros<cube>(G,m,y); 
  arma::rowvec d1=d;
  ...
  for(miter=0; miter<500; miter++){
    ...
    if (len3>0){
      ...
      for (int mm=2; mm<2+len3; mm++){
        arma::mat bet000=bet0.elem(find(bet!=0));
        arma::mat bet00=bet.elem(find(bet!=0));
        arma::mat betmle0=betmle.elem(find(bet!=0));
        add(mm)=soft2(bet000(mm-2,0),-(eta/pow(abs(betmle0(mm-2,0)),lam))/FI2(mm,mm))-bet00(mm-2,0);
      }
      bet1.elem(find(bet!=0))=bet.elem(find(bet!=0))+add.subvec(2,2+len3-1).t();
    }
