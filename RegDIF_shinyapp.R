...
Reg_VEMM_DIF <- function(resp,indic,lbd,eta,eps,Group,Unif,domain,y,G,N.vec,gra00,grd00,grbeta00,grgamma00,Mu.list,Sig.list,cons)
{
  ...
  l0norm=l0norm1+l0norm2
  GIC=-2*ll+cons*log(log(person))*log(person)*l0norm
  ...
  return(list(est=cbind(new_a,new_b),Gamma=new_gamma,Beta=new_beta,n=n,bic=BIC,gic=GIC,Likelihood=ll, means=Mu.est,Covs=Sigma.est))
}
