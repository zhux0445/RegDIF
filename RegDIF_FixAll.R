...
######################################################################## 
#####      constraint on beta for each group separatly
########################################################################
#  Beta:
#  0.5  0
#   0   1
...
Dmat3=matrix(params[,9],J,(m-1))
# beta_11=0.5
Dmat2[1,1]=Dmat2[1,1]+0.5
# beta_22=1
Dmat3[2,1]=Dmat3[2,1]+1
...
theta.ng1=theta[1:N1,]
theta.ng1[which(theta[1:N1,1]<X1[1]),1]=-3
theta.ng1[which(theta[1:N1,2]<X1[1]),2]=-3
for (gg in 1:(length(X1)-1)){
  theta.ng1[which(theta[1:500,1]>X1[gg]&theta[1:500,1]<X1[gg+1]),1]=X1[gg]
  theta.ng1[which(theta[1:500,2]>X1[gg]&theta[1:500,2]<X1[gg+1]),2]=X1[gg]
}
...
grbeta=matrix(0,J,2)
grbeta[,1]=matrix(c(0.7,0,rep(0.7,18)),J,1)
grbeta[,2]=matrix(c(0,rep(1.4,19)),J,1)
#####################
gra=as.matrix(Amat1)
rownames(gra) <- c()
grd=Dmat1
...
while(max(df.beta)>eps) # max(df.Mu)>eps | max(df.Sig)>eps |
{
  ...
  for (j in 1:2)
  {
    ...
    while(sum(abs(add))>max.tol)
    {
      ...
      #calculating the score vector
      Betsco <- c(sum(apply(rgk2/P2,1,diff)*Pstar2*Qstar2),sum(apply(rgk3/P3,1,diff)*Pstar3*Qstar3))[j]
      minusgrad <- -Betsco
      FI <- c(-sum(ng2*(Pstar2[,1]*Qstar2[,1])^2*(1/P2[,1]+1/P2[,2])),-sum(ng3*(Pstar3[,1]*Qstar3[,1])^2*(1/P3[,1]+1/P3[,2])))[j]      
      add <- minusgrad/FI
      bet[j]=bet[j]+add
    }
    #end of M step loop
