...
for (rep in 1:reps){
  ...
  print(biass.2[rep,])
  print(RMSEs.2[rep,])
  write.csv(eta.2[rep],file = paste("eta2LowCor_",rep))
  write.csv(ADmat.2[,,rep],file = paste("ADmat2LowCor_",rep))
  write.csv(Betas.2[,,rep],file = paste("Beta2LowCor_",rep))
  write.csv(theta.dist[,,kk],file = paste("theta2LowCor_",rep))
}
#sim2 lower EM
for (rep in 1:reps){
  resp=responses[((rep-1)*N+1):((rep-1)*N+N1+N2+N3),]
